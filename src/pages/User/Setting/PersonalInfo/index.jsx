    ...
    <div className="container md:px-60 px-4 mx-auto mt-8">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/account-settings">Tài khoản</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
        ...
        <div className="col-span-2">
          <div className="py-6 border-b">
            <div className=" flex justify-between ">
              <div>
                <div className="text-base text-[#222]">Tên đầy đủ</div>
                {!edit.isEditFullName && (
                ...
                {edit.isEditFullName && (
                  <div className="text-[#717171]">
                    Tên của bạn sẽ hiển thị như trên cho Chủ nhà và khách.
                  </div>
                )}
              </div>
              <div
                className="underline font-semibold text-xs cursor-pointer"
                onClick={() => handleToggleSetEdit("isEditFullName")}
              >
                {edit.isEditFullName
                  ? "Đóng"
                  : user?.fullName
            ...
            {edit.isEditFullName && <EditFullName onSubmit={onSubmit} />}
          </div>
          <div className="py-6  border-b">
            <div className="flex justify-between">
              <div>
                <div className="text-base text-[#222]">Tên đăng nhập</div>
                {!edit.isEditUsername && (
                  <div className="text-[#717171]">
              ...
              <div
                className="underline font-semibold text-xs cursor-pointer"
                onClick={() => handleToggleSetEdit("isEditUsername")}
              >
                {edit.isEditUsername
                  ? "Đóng"
                  : user?.username
            ...
            {edit.isEditUsername && <EditUsername onSubmit={onSubmit} />}
          </div>
          <div className="py-6 border-b">
            <div className="flex justify-between ">
              <div>
                <div className="text-base text-[#222]">Địa chỉ email</div>
                {!edit.isEditEmail && (
                  <div className="text-[#717171]">
                ...
                {edit.isEditEmail && (
                  <div className="text-[#717171]">
                    Sử dụng địa chỉ mà bạn luôn có quyền truy cập.
                  </div>
                )}
              </div>
              <div
                className="underline font-semibold text-xs cursor-pointer"
                onClick={() => handleToggleSetEdit("isEditEmail")}
              >
                {edit.isEditEmail ? "Đóng" : user?.email ? "Chỉnh sửa" : "Thêm"}
              </div>
            </div>
          ...
          <div className="py-6  border-b">
            <div className="flex justify-between">
              <div>
                <div className="text-base text-[#222]">Số điện thoại</div>
                {!edit.isEditPhoneNumber && (
                  <div className="text-[#717171]">
                    {user?.phoneNumber || "Chưa được cung cấp"}
                  </div>
                )}
                {edit.isEditPhoneNumber && (
                  <div className="text-[#717171]">
                    Thêm số điện thoại để khách đã xác nhận và{" "}
                    <strong>trovn</strong> có thể liên hệ với bạn. Bạn có thể
                    thêm các số điện thoại khác và chọn mục đích sử dụng tương
                    ứng.
                  </div>
                )}
              </div>
              <div
                className="underline font-semibold text-xs cursor-pointer"
                onClick={() => handleToggleSetEdit("isEditPhoneNumber")}
              >
                {edit.isEditPhoneNumber
                  ? "Đóng"
                  : user?.phoneNumber
            ...
            {edit.isEditPhoneNumber && <EditPhoneNumber onSubmit={onSubmit} />}
          </div>
