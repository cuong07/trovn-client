    ...
    >
      ...
      <div className="pl-10 flex gap-2 mt-2">
        <div>
          <LazyLoadImage
            effect="blur"
            src={notification.data.listing.images[0].url}
            className="size-16 min-w-16 rounded-md "
          />
        </div>
        <div>
          <p className="font-semibold text-base">
            {notification.data.listing.title}
          </p>
          <p>{notification.data.listing.address}</p>
        </div>
      </div>
      {!notification.isRead && (
...
export default RentedRoomCanceled;
